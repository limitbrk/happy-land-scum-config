...
- `Conditions`: เงื่อนไขที่ต้องทำให้สำเร็จเพื่อเคลียร์เควส(สามารถใส่พร้อมกันได้) โดยตั้งค่าเงื่อนไขพื้นฐาน มีดังนี้
  ...
  ```
  - `CanBeAutoCompleted`: ถ้าเป็น `true` จะสามารถเคลียร์เควสได้โดยไม่ต้องกลับไปส่งเควส (เหมือนตอนนี้จะนับจากแค่SequenceIndex 0 เท่านั้น)
  ...
  - `Type`: ประเภทเงื่อนไขที่ต้องทำให้สำเร็จ มี 3 ประเภทคือ
    ...
    ```
    - `Interaction`: โต้ตอบกับวัตถุในเกม โดยสามารถใช้ `#GetMeshInfo` เพื่อดูวัตถุที่ใช้โต้ตอบที่มองอยู่ได้ (โดยสามารถโต้ตอบได้ 1 ชิ้นต่อ 1 ครั้ง) จะได้ Json มาใส่ใน `Locations` ดังนี้
      ...
      - `SpawnOnlyNeeded`: 
        ...
        - หากตั้งค่าเป็น `false`: → วัตถุเป้าหมายทั้งหมดในรายการจะถูกสร้างขึ้นมา แต่ผู้เล่นไม่จำเป็นต้องโต้ตอบกับทั้งหมด—แค่ครบจำนวนที่กำหนดไว้ก็เพียงพอ
    ...
    ```
    กรณี่อยากให้้เป็นวัตตุที่ผู้เล่นสร้างขึ้นเองเช่น Checkpoint ลองใช้
    ```json
    {
        "Locations": [
            {
